...
func registerAllTools(s *server.MCPServer) {
	// --- Cluster (5) ---
	registerTool(s,tool("cluster_health", "Check if the Cube Container cluster API is reachable and healthy."), handleClusterHealth)
	registerTool(s,tool("cluster_overview", "Get cluster capacity overview: total nodes, running containers, CPU/RAM usage."), handleClusterOverview)
	registerTool(s,tool("cluster_versions", "Get version matrix of all cluster components (CubeAPI, CubeMaster, Cubelet)."), handleClusterVersions)
	registerTool(s,tool("list_nodes", "List all nodes in the cluster with their resource capacity and current load."), handleListNodes)
	registerTool(s,toolWithArgs("get_node", "Get detailed info for a specific node.",
		mcp.WithString("node_id", mcp.Required(), mcp.Description("Node ID")),
	), handleGetNode)
	registerTool(s,toolWithArgs("suggest_node", "Suggest the best node for a new container based on resource availability. Returns top-3 candidates with bin-packing scores. Pass required memory_mb and cpu_count to filter.",
	...
	registerTool(s,toolWithArgs("list_containers", "List running containers (sandboxes) with optional filters. Args: state (running/paused/stopped), limit (default 50).",
		mcp.WithString("state", mcp.Description("Filter: running, paused, or stopped")),
		mcp.WithNumber("limit", mcp.Description("Max results (default 50)")),
	), handleListContainers)
	registerTool(s,toolWithArgs("get_container", "Get detailed info for a specific container by ID.",
		mcp.WithString("container_id", mcp.Required()),
	), handleGetContainer)
	registerTool(s,toolWithArgs("create_container", "Create and start a new container from a template. Args: template_id (required), memory_mb (default 512), cpu_count (default 1.0), env_vars, metadata, configmap (name of a ConfigMap whose keys are injected as env vars).",
		...
		mcp.WithString("configmap", mcp.Description("Name of a ConfigMap to inject as environment variables")),
	), handleCreateContainer)
	registerTool(s,toolWithArgs("kill_container", "Stop and remove a container by ID.",
		mcp.WithString("container_id", mcp.Required()),
	), handleKillContainer)
	registerTool(s,toolWithArgs("pause_container", "Freeze a container (cgroup freezer). Uses ~0 CPU while paused. Resume with resume_container.",
		mcp.WithString("container_id", mcp.Required()),
	), handlePauseContainer)
	registerTool(s,toolWithArgs("resume_container", "Resume (un-freeze) a paused container. Typically resumes in ~15ms.",
		mcp.WithString("container_id", mcp.Required()),
	), handleResumeContainer)
	registerTool(s,toolWithArgs("get_container_logs", "Fetch recent logs from a container.",
		mcp.WithString("container_id", mcp.Required()),
		mcp.WithNumber("limit", mcp.Description("Max log lines (default 100)")),
	), handleGetContainerLogs)
	registerTool(s,toolWithArgs("tail_container_logs", "Get the last N log lines from a container (one-shot). For real-time streaming use the SSE endpoint at GET /streams/{container_id}/logs.",
		mcp.WithString("container_id", mcp.Required()),
		mcp.WithNumber("limit", mcp.Description("Number of lines (default 50)")),
	), handleTailLogs)

	// --- Templates (3) ---
	registerTool(s,tool("list_templates", "List all available container templates."), handleListTemplates)
	registerTool(s,toolWithArgs("create_template", "Create a new template from an OCI image. Args: image (required, e.g. 'python:3.12-slim'), expose_ports, writable_layer_size_gb.",
		mcp.WithString("image", mcp.Required(), mcp.Description("OCI image reference")),
	), handleCreateTemplate)
	registerTool(s,toolWithArgs("get_template", "Get details of a specific template.",
		mcp.WithString("template_id", mcp.Required()),
	), handleGetTemplate)

	// --- Persistent deploy (4) ---
	registerTool(s,toolWithArgs("deploy_from_git", "Deploy from a git repo with persistent storage. Flow: clone → volume → template → container. Code survives restarts.",
	...
	registerTool(s,toolWithArgs("deploy_from_code", "Deploy from inline code files (no git needed). Files written to persistent volume.",
		mcp.WithString("app_name", mcp.Required()),
	), handleDeployFromCode)
	registerTool(s,toolWithArgs("update_code", "Pull latest code from git and sync to the container's volume. Sends restart signal.",
		mcp.WithString("container_id", mcp.Required()),
		mcp.WithString("git_url", mcp.Required()),
		mcp.WithString("branch", mcp.Description("Git branch (default main)")),
	), handleUpdateCode)
	registerTool(s,toolWithArgs("exec_in_container", "Execute a command inside a running container. Returns stdout, stderr, exit code.",
		mcp.WithString("container_id", mcp.Required()),
	...
	// --- Volumes (3) ---
	registerTool(s,tool("list_volumes", "List all persistent volumes with size and file count."), handleListVolumes)
	registerTool(s,toolWithArgs("create_volume", "Create a new persistent volume directory.",
		mcp.WithString("name", mcp.Required()),
	), handleCreateVolume)
	registerTool(s,toolWithArgs("delete_volume", "Delete a persistent volume. WARNING: destroys all data permanently.",
		mcp.WithString("name", mcp.Required()),
	), handleDeleteVolume)

	// --- Volume lifecycle: attach/detach/migrate/info (4) ---
	registerTool(s,toolWithArgs("volume_attach", "Attach a persistent volume to a running container at a specified mount path.",
		mcp.WithString("container_id", mcp.Required()),
		mcp.WithString("volume_name", mcp.Required()),
		mcp.WithString("mount_path", mcp.Required()),
	), handleVolumeAttach)
	registerTool(s,toolWithArgs("volume_detach", "Detach a volume from a container. The volume data is preserved.",
		mcp.WithString("container_id", mcp.Required()),
		mcp.WithString("volume_name", mcp.Required()),
	), handleVolumeDetach)
	registerTool(s,toolWithArgs("volume_migrate", "Migrate a volume from this node to a remote node via tar+scp. Requires SSH access to the target node.",
		mcp.WithString("volume_name", mcp.Required()),
	...
	registerTool(s,toolWithArgs("volume_info", "Get detailed volume info: size, file count, attached containers.",
		mcp.WithString("volume_name", mcp.Required()),
	), handleVolumeInfo)

	// --- Resource limits (4) ---
	registerTool(s,toolWithArgs("resource_set_limits", "Apply hard memory and CPU limits to a running container via docker update. Limits are persisted and re-applied on restart.",
		mcp.WithString("container_id", mcp.Required()),
	...
	registerTool(s,toolWithArgs("resource_get_usage", "Get real-time CPU, memory, network, and disk I/O usage for a specific container.",
		mcp.WithString("container_id", mcp.Required()),
	), handleResourceGetUsage)
	registerTool(s,tool("resource_list_usage", "Get real-time resource usage for ALL running containers. Returns CPU%, memory, network I/O, and PID count per container."), handleResourceListUsage)
	registerTool(s,tool("resource_quota_summary", "Get aggregated resource allocation vs node capacity: total allocated memory/CPU, utilization percentage, and remaining headroom."), handleResourceQuotaSummary)

	// --- Garbage collection (3) ---
	registerTool(s,tool("gc_prune_images", "Remove unused and dangling Docker images older than 7 days. Frees disk space on edge nodes with limited storage."), handleGCPruneImages)
	registerTool(s,tool("gc_prune_volumes", "Remove orphaned Docker volumes not attached to any running container. Also identifies deploy-managed volumes with no active container."), handleGCPruneVolumes)
	registerTool(s,tool("gc_disk_usage", "Show disk usage breakdown: images, containers, volumes — total count, active, size, and reclaimable space."), handleGCDiskUsage)

	// --- Backup & Restore (5) ---
	registerTool(s,toolWithArgs("backup_volume", "Create a tar.gz backup of a volume with SHA256 integrity check. Backup is stored locally and can be restored later.",
		mcp.WithString("volume_name", mcp.Required(), mcp.Description("Name of the volume to backup")),
	), handleBackupVolume)
	registerTool(s,toolWithArgs("backup_container", "Create a full backup of a container: config manifest + all mounted volumes. Allows full recovery on restore.",
		mcp.WithString("container_id", mcp.Required()),
	), handleBackupContainer)
	registerTool(s,tool("list_backups", "List all available backups with size, checksum, and restorable status."), handleListBackups)
	registerTool(s,toolWithArgs("restore_backup", "Restore a backup by ID. Verifies SHA256 integrity before restoring. For containers, recreates the container from manifest.",
		mcp.WithString("backup_id", mcp.Required()),
	), handleRestoreBackup)
	registerTool(s,toolWithArgs("delete_backup", "Delete a backup file and its manifest permanently.",
		mcp.WithString("backup_id", mcp.Required()),
	), handleDeleteBackup)

	// --- Deploy versioning & rollback (2) ---
	registerTool(s,toolWithArgs("rollback_deploy", "Rollback a deployment to its previous version. Redeploys from the prior git commit.",
		mcp.WithString("app_name", mcp.Required()),
	), handleRollbackDeploy)
	registerTool(s,toolWithArgs("list_deploy_versions", "List all deployment versions for an app.",
		mcp.WithString("app_name", mcp.Required()),
	), handleListVersions)

	// --- Routing & automatic TLS (3) ---
	registerTool(s,toolWithArgs("create_route", "Create a domain route with automatic TLS. Caddy obtains Let's Encrypt certificates automatically. The domain must point to this server's IP.",
	...
	registerTool(s,toolWithArgs("delete_route", "Remove a domain route and its TLS certificate.",
		mcp.WithString("domain", mcp.Required()),
	), handleDeleteRoute)
	registerTool(s,tool("list_routes", "List all configured domain routes with TLS status."), handleListRoutes)
	registerTool(s,tool("reload_routes", "Force regenerate the Caddy route config and reload Caddy. Use after manual config changes or if routes appear stale."), handleReloadRoutes)

	// --- Networking (9) ---
	registerTool(s,toolWithArgs("add_port_mapping", "Map a host port to a container port. Allows external access to a container service.",
		mcp.WithString("container_id", mcp.Required()),
	...
	registerTool(s,toolWithArgs("remove_port_mapping", "Remove a port mapping by ID.",
		mcp.WithString("mapping_id", mcp.Required()),
	), handleRemovePortMapping)
	registerTool(s,tool("list_port_mappings", "List all port mappings."), handleListPortMappings)
	registerTool(s,toolWithArgs("add_dns_alias", "Add a DNS alias pointing to a container. Creates /etc/hosts entry.",
	...
	registerTool(s,toolWithArgs("remove_dns_alias", "Remove a DNS alias.",
		mcp.WithString("alias", mcp.Required()),
	), handleRemoveDNSAlias)
	registerTool(s,tool("list_dns_aliases", "List all DNS aliases."), handleListDNSAliases)
	registerTool(s,toolWithArgs("add_network_policy", "Add a network policy (allow/deny) between containers.",
		mcp.WithString("name", mcp.Required()),
	...
	registerTool(s,tool("list_network_policies", "List all network policies."), handleListNetworkPolicies)
	registerTool(s,toolWithArgs("remove_network_policy", "Remove a network policy by ID.",
	...
	registerTool(s,tool("backend_info", "Get information about the active container backend (docker or cube). Returns backend name, endpoint, and capabilities. Use this to understand which container runtime the MCP server is operating on."), handleBackendInfo)

	// --- Multi-node cluster management (6) ---
	registerTool(s,toolWithArgs("node_add", "Register a new cluster node. Args: id (required, unique identifier), address (required, host:port), backend (docker or cube), hostname, memory_mb, cpu_cores, disk_gb, state (active/draining/offline, default active).",
	...
	registerTool(s,toolWithArgs("node_update", "Update a registered node's properties (address, state, resources).",
		mcp.WithString("id", mcp.Required()),
	), handleNodeUpdate)
	registerTool(s,toolWithArgs("node_remove", "Remove a node from the cluster registry. Containers on the node are NOT affected.",
		mcp.WithString("id", mcp.Required()),
	), handleNodeRemove)
	registerTool(s,tool("node_list", "List all registered cluster nodes with state, resources, and backend type."), handleNodeList)
	registerTool(s,toolWithArgs("node_get", "Get detailed information about a specific node.",
		mcp.WithString("id", mcp.Required()),
	), handleNodeGet)
	registerTool(s,toolWithArgs("deploy_to_node", "Deploy a container to a specific remote node. Creates a remote backend connection to the node and runs the container there. Use suggest_node first to pick the best node.",
	...
	registerTool(s,toolWithArgs("scale_set", "Set the exact number of replicas for a service. Creates or removes containers to match the desired count. If the service has a domain, the load balancer is updated automatically.",
		mcp.WithString("name", mcp.Required()),
		mcp.WithNumber("replicas", mcp.Required(), mcp.Description("Desired replica count (0-20)")),
	), handleScaleSet)
	registerTool(s,tool("service_list", "List all scalable services with current replica counts and container IDs."), handleServiceList)
	registerTool(s,toolWithArgs("service_get", "Get detailed information about a scalable service.",
		mcp.WithString("name", mcp.Required()),
	), handleServiceGet)
	registerTool(s,toolWithArgs("service_remove", "Remove a service definition. Containers are NOT killed automatically.",
		mcp.WithString("name", mcp.Required()),
	), handleServiceRemove)

	// --- Service discovery (4) ---
	registerTool(s,toolWithArgs("service_register", "Register or update a service endpoint in the discovery registry. Maps a logical name to a container's host:port. Args: name (required), container_id (required), host (default localhost), port (required).",
		mcp.WithString("name", mcp.Required()),
		mcp.WithString("container_id", mcp.Required()),
		mcp.WithString("host", mcp.Description("Host (default localhost)")),
		mcp.WithNumber("port", mcp.Required()),
	), handleDiscoveryRegister)
	registerTool(s,toolWithArgs("service_deregister", "Remove a service from the discovery registry.",
		mcp.WithString("name", mcp.Required()),
	), handleDiscoveryDeregister)
	registerTool(s,toolWithArgs("service_resolve", "Look up a service by logical name. Returns host:port.",
		mcp.WithString("name", mcp.Required()),
	), handleDiscoveryResolve)
	registerTool(s,toolWithArgs("service_entries", "List all registered service discovery entries. Pass sync=true to first refresh from running container labels.",
		mcp.WithString("sync", mcp.Description("Set to 'true' to sync from container labels first")),
	), handleServiceListEntries)

	// --- High availability (1) ---
	registerTool(s,tool("ha_state", "Get the current high-availability state of this CubeMaster node: role (active/standby), active node ID, peer health, and failover timing."), handleHAState)

	// --- Health checks & auto-restart (4) ---
	registerTool(s,toolWithArgs("health_check_set", "Configure a health check probe for a container. Supports HTTP (GET 2xx/3xx), TCP (connection), and exec (exit 0) probes. When the probe fails consecutively beyond the threshold, the container is automatically restarted. Args: container_id (required), type (http|tcp|exec, required), interval_seconds (default 30), timeout_seconds (default 5), failure_threshold (default 3). For http: host, http_port, http_path (default /), http_scheme (default http). For tcp: host, tcp_port. For exec: exec_command.",
		mcp.WithString("container_id", mcp.Required()),
	...
	registerTool(s,toolWithArgs("health_check_remove", "Remove a health check probe from a container. Auto-restart will stop.",
		mcp.WithString("container_id", mcp.Required()),
	), handleHealthCheckRemove)
	registerTool(s,tool("health_check_list", "List all configured health checks with current status, failure counts, and restart counts."), handleHealthCheckList)
	registerTool(s,toolWithArgs("health_check_status", "Get detailed health check status for a specific container, including last error and probe configuration.",
		mcp.WithString("container_id", mcp.Required()),
	), handleHealthCheckStatus)

	// --- Secrets management (4) ---
	registerTool(s,toolWithArgs("secret_set", "Store an encrypted secret (API keys, passwords, tokens). Value is encrypted at rest with AES-256-GCM. RBAC: admin only.",
		mcp.WithString("name", mcp.Required()),
		mcp.WithString("value", mcp.Required()),
	), handleSecretSet)
	registerTool(s,toolWithArgs("secret_get", "Decrypt and retrieve a secret by name. RBAC: operator.",
		mcp.WithString("name", mcp.Required()),
	), handleSecretGet)
	registerTool(s,tool("secret_list", "List all stored secrets with metadata (name, version, timestamps). Does NOT reveal values. RBAC: viewer."), handleSecretList)
	registerTool(s,toolWithArgs("secret_delete", "Permanently delete a secret. RBAC: admin.",
		mcp.WithString("name", mcp.Required()),
	), handleSecretDelete)

	// --- Alerting (4) ---
	registerTool(s,toolWithArgs("alert_rule_add", "Create an alert rule. Types: container_down, cpu_high, disk_high, mem_high. Args: id (required), name (required), type (required), severity (critical/warning/info, default warning), threshold (e.g. 80 for 80%), container_id (for container_down), node_id (for cpu/disk/mem), webhook_url (optional override), cooldown_sec (default 300).",
		mcp.WithString("id", mcp.Required()),
		mcp.WithString("name", mcp.Required()),
	...
	registerTool(s,toolWithArgs("alert_rule_remove", "Remove an alert rule.",
		mcp.WithString("id", mcp.Required()),
	), handleAlertRuleRemove)
	registerTool(s,tool("alert_list", "List all alert rules with severity, fire count, and last fired timestamp."), handleAlertList)
	registerTool(s,toolWithArgs("alert_test", "Fire a test alert to verify webhook configuration.",
		mcp.WithString("id", mcp.Required()),
	), handleAlertTest)

	// --- ConfigMaps (5) ---
	registerTool(s,toolWithArgs("configmap_create", "Create a ConfigMap with non-sensitive configuration data (env vars, feature flags). For sensitive data use secret_set instead.",
		mcp.WithString("name", mcp.Required()),
	), handleConfigMapCreate)
	registerTool(s,toolWithArgs("configmap_update", "Update a ConfigMap. By default merges new keys with existing data. Use mode=replace to overwrite entirely.",
		mcp.WithString("name", mcp.Required()),
		mcp.WithString("mode", mcp.Description("merge (default) or replace")),
	), handleConfigMapUpdate)
	registerTool(s,toolWithArgs("configmap_get", "Get a ConfigMap with all its data values.",
		mcp.WithString("name", mcp.Required()),
	), handleConfigMapGet)
	registerTool(s,tool("configmap_list", "List all ConfigMaps with key count and last updated timestamp."), handleConfigMapList)
	registerTool(s,toolWithArgs("configmap_remove", "Remove a ConfigMap permanently.",
		mcp.WithString("name", mcp.Required()),
	), handleConfigMapRemove)

	// --- Image lifecycle (5) — CI/CD pipeline: build → push → pull → tag ---
	registerTool(s,toolWithArgs("image_build", "Build a Docker image from a Dockerfile in a context directory. Sends the context as a tarball to the Docker build API. Args: context_dir (required, path to directory containing Dockerfile), tag (required, e.g. myapp:v1), dockerfile (default Dockerfile).",
	...
	registerTool(s,toolWithArgs("image_push", "Push an image to a registry. The image is tagged with the registry prefix first. Args: tag (required), registry (optional, e.g. registry.example.com:5000).",
		mcp.WithString("tag", mcp.Required()),
		mcp.WithString("registry", mcp.Description("Registry URL (default: Docker Hub)")),
	), handleImagePush)
	registerTool(s,toolWithArgs("image_pull", "Pull an image from a registry. Args: tag (required, e.g. postgres:16-alpine).",
		mcp.WithString("tag", mcp.Required()),
	), handleImagePull)
	registerTool(s,toolWithArgs("image_list", "List all Docker images on the node with tags, size, and creation date.",
		mcp.WithString("filter", mcp.Description("Filter images by name pattern")),
	), handleImageList)
	registerTool(s,toolWithArgs("image_tag", "Tag an existing image with a new name:tag. Args: source_tag (required), target_tag (required).",
	...
	registerTool(s,toolWithArgs("logs_search", "Search logs across ALL (or specified) containers by text pattern, severity level, or time range. Returns matching log lines sorted by recency. Args: pattern (text to search), containers (list of IDs, empty=all), level (error|warn|info|debug), since_minutes, max_results (default 100).",
	...
	registerTool(s,toolWithArgs("logs_aggregate", "Aggregate log statistics across containers. Returns error/warn/info counts per container, sorted by error count. Args: containers (list of IDs, empty=all), since_lines (lines to analyze per container, default 200).",
		mcp.WithString("level", mcp.Description("Aggregate specific level only")),
	), handleLogsAggregate)

	// --- Audit query (1) — investigate what happened ---
	registerTool(s,toolWithArgs("audit_query", "Search the tamper-evident audit log. Every tool invocation is recorded with a hash chain. Query by time range, tool name, role, or success/failure. Args: since_hours (default 24), tool_name, role, success (true/false), limit (default 100).",
	...
	registerTool(s,toolWithArgs("env_create", "Create an isolated environment (namespace) for managing deployments. Default environments (dev, staging, prod) are auto-created. Args: name (required), description, protected (bool).",
		mcp.WithString("name", mcp.Required()),
		mcp.WithString("description", mcp.Description("Environment description")),
	), handleEnvCreate)
	registerTool(s,tool("env_list", "List all environments with container counts."), handleEnvList)
	registerTool(s,toolWithArgs("env_get", "Get details of a specific environment including its containers.",
		mcp.WithString("name", mcp.Required()),
	), handleEnvGet)
	registerTool(s,toolWithArgs("env_promote", "Promote a container from one environment to the next (e.g. dev → staging → prod). Creates a new container in the target with the same image, then removes the source. Args: source_env (required), target_env (required), container_id (required).",
	...
	registerTool(s,toolWithArgs("notify_channel_add", "Register a notification channel for alert delivery. Supports Slack, Discord, Telegram, and Email. Args: name (required), type (slack|discord|telegram|email, required), webhook_url (for slack/discord), bot_token + chat_id (for telegram), email_to + smtp_host (for email).",
		mcp.WithString("name", mcp.Required()),
	...
	registerTool(s,tool("notify_channel_list", "List all configured notification channels."), handleNotifyChannelList)
	registerTool(s,toolWithArgs("notify_channel_remove", "Remove a notification channel.",
		mcp.WithString("channel_id", mcp.Required()),
	), handleNotifyChannelRemove)
	registerTool(s,toolWithArgs("notify_send", "Send a message to a notification channel. Args: channel_id (required), title (required), body (required), level (info|warning|critical).",
		mcp.WithString("channel_id", mcp.Required()),
	...
	registerTool(s,toolWithArgs("auth_create_token", "Create a new API token with a specified role. The secret is returned ONLY once — store it securely. Args: role (required: viewer|operator|admin), label.",
	...
	registerTool(s,tool("auth_list_tokens", "List all API tokens with metadata (no secrets shown)."), handleAuthListTokens)
	registerTool(s,toolWithArgs("auth_revoke_token", "Revoke (disable) an API token by its key ID.",
		mcp.WithString("key", mcp.Required()),
	), handleAuthRevokeToken)

	// --- Scheduled jobs (4) — periodic task automation ---
	registerTool(s,toolWithArgs("job_create", "Create a scheduled job that runs an MCP tool periodically. Args: name (required), schedule (required, e.g. 'every 5m', 'hourly', 'daily', 'weekly'), tool (required, MCP tool name), args (object of tool arguments).",
		mcp.WithString("name", mcp.Required()),
	...
	registerTool(s,tool("job_list", "List all scheduled jobs with next run time and status."), handleJobList)
	registerTool(s,toolWithArgs("job_remove", "Remove a scheduled job.",
		mcp.WithString("job_id", mcp.Required()),
	), handleJobRemove)
	registerTool(s,toolWithArgs("job_run", "Run a scheduled job immediately (outside its schedule).",
		mcp.WithString("job_id", mcp.Required()),
	), handleJobRun)

	// --- Metrics query (1) — programmatic metrics access ---
	registerTool(s,toolWithArgs("metrics_query", "Query current cluster metrics (CPU, memory, requests, tool calls). Filter by metric name or container. Args: metric (name pattern), container (filter), limit (default 50).",
	...
	registerTool(s,toolWithArgs("database_create", "Provision a managed database (Postgres, MySQL, Redis, MongoDB). Creates container, persistent volume, health check, and stores credentials as a secret. Args: name (required), type (required: postgres|mysql|redis|mongodb), version (e.g. 16), memory_mb (default 512).",
		mcp.WithString("name", mcp.Required()),
	...
	registerTool(s,toolWithArgs("database_backup", "Create a backup of a managed database's volume.",
		mcp.WithString("database_id", mcp.Required()),
	), handleDatabaseBackup)
	registerTool(s,toolWithArgs("database_restore", "Restore a managed database from a backup.",
		mcp.WithString("database_id", mcp.Required()),
		mcp.WithString("backup_id", mcp.Required()),
	), handleDatabaseRestore)

	// --- Certificates (2) — TLS visibility ---
	registerTool(s,tool("cert_list", "List all TLS certificates with expiry dates and renewal status. Scans Caddy certificate store and CUBE_TLS_CERT."), handleCertList)
	registerTool(s,toolWithArgs("cert_renew", "Trigger certificate renewal check by reloading Caddy. Caddy auto-renews certs expiring within 30 days.",
		mcp.WithString("domain", mcp.Required()),
	), handleCertRenew)

	// --- Events (2) — cluster event stream ---
	registerTool(s,toolWithArgs("events_list", "List recent cluster events (container starts/stops, deploys, scaling, health failures, alerts). Filter by type, severity, or time range. Args: event_type, severity (info|warning|critical), since_minutes, limit (default 50).",
	...
	registerTool(s,tool("events_recent", "Get the 20 most recent cluster events across all types."), handleEventsRecent)
	...
	registerTool(s,toolWithArgs("secure_sandbox_create", "Create a KVM-isolated sandbox for untrusted code. Each sandbox runs its own guest kernel — kernel escape is impossible. Supports egress filtering (domain allowlist/blocklist) and credential vault (API keys injected by proxy, never visible to sandbox code). Requires Cube backend. Args: template_id (required), memory_mb (default 512), cpu_count (default 1.0), egress_allowlist (domains the sandbox can reach), egress_blocklist (domains to block), credential_vault (map of domain→API key, injected on egress), max_lifetime_seconds (auto-pause after N seconds), network_disabled (bool, most secure).",
		mcp.WithString("template_id", mcp.Required()),
		mcp.WithNumber("memory_mb", mcp.Description("Memory limit in MB (default 512)")),
		mcp.WithNumber("cpu_count", mcp.Description("CPU cores (default 1.0)")),
	), handleSecureSandboxCreate)
	registerTool(s,toolWithArgs("secure_sandbox_exec", "Execute arbitrary code inside a secure KVM-isolated sandbox. By design, NO command allowlist is applied — isolation is at the VM kernel level. Egress filtering is enforced. Max timeout 300s. Args: sandbox_id (required), command (required), timeout_seconds (default 30, max 300).",
	...
	registerTool(s,toolWithArgs("secure_sandbox_egress_add", "Add a network egress rule to a secure sandbox. Only domains in the allowlist can be reached from inside the sandbox. Args: sandbox_id (required), domain (required), action (allow|block, required).",
		mcp.WithString("sandbox_id", mcp.Required()),
		mcp.WithString("domain", mcp.Required()),
		mcp.WithString("action", mcp.Required(), mcp.Description("allow or block")),
	), handleSecureSandboxEgressAdd)
	registerTool(s,toolWithArgs("secure_sandbox_egress_list", "List all egress rules for a secure sandbox.",
		mcp.WithString("sandbox_id", mcp.Required()),
	), handleSecureSandboxEgressList)
	registerTool(s,toolWithArgs("secure_sandbox_egress_remove", "Remove an egress rule from a secure sandbox.",
		mcp.WithString("rule_id", mcp.Required()),
	), handleSecureSandboxEgressRemove)
	registerTool(s,toolWithArgs("secure_sandbox_snapshot", "Create a CubeCoW snapshot of a secure sandbox. Allows instant rollback to a known-good state. Copy-on-Write means snapshots are near-instant and minimal storage. Args: sandbox_id (required).",
		mcp.WithString("sandbox_id", mcp.Required()),
	), handleSecureSandboxSnapshot)
	registerTool(s,toolWithArgs("secure_sandbox_restore", "Restore a secure sandbox to a previous snapshot. Rolls back the entire sandbox state (filesystem + memory) to the snapshot point. Args: sandbox_id (required), snapshot_id (required).",
		mcp.WithString("sandbox_id", mcp.Required()),
