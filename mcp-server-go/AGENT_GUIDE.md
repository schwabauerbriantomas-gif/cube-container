...
2. **Handlers in `handlers_phase2.go`** (or end of `server.go`). Feature files contain logic only.
...
5. **Mutex-protect all shared state** — use `sync.Mutex` or `sync.RWMutex`, never bare maps.
6. **Persist state to disk** — managers that hold state must survive restarts.
7. **No external dependencies** unless absolutely necessary — Go stdlib + mcp-go only.
8. **Comments in English** — but the code speaks for itself. Document WHY, not WHAT.
...
[ ] 5. Registered tool in registerAllTools() (server.go)
